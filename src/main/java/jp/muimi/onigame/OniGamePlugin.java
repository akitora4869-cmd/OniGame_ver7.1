...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private ItemStack registeredEquipment(String id){
        ...
        m.getPersistentDataContainer().set(equipmentIdKey,PersistentDataType.STRING,d.id());m.getPersistentDataContainer().set(actionKey,PersistentDataType.STRING,"equipment:"+d.id());
    ...
    private String oniArmorId(OniType type,String slot){return "oni."+type.name().toLowerCase(Locale.ROOT)+"."+slot;}
    private void equipOniArmor(Player p,OniType type){p.getInventory().setHelmet(registeredEquipment(oniArmorId(type,"helmet")));p.getInventory().setChestplate(registeredEquipment(oniArmorId(type,"chestplate")));p.getInventory().setLeggings(registeredEquipment(oniArmorId(type,"leggings")));p.getInventory().setBoots(registeredEquipment(oniArmorId(type,"boots")));}
    private void equipOniArmor(LivingEntity e,OniType type){if(e.getEquipment()==null)return;e.getEquipment().setHelmet(registeredEquipment(oniArmorId(type,"helmet")));e.getEquipment().setChestplate(registeredEquipment(oniArmorId(type,"chestplate")));e.getEquipment().setLeggings(registeredEquipment(oniArmorId(type,"leggings")));e.getEquipment().setBoots(registeredEquipment(oniArmorId(type,"boots")));}
    private void equipDakkoArmor(Player p){equipOniArmor(p,OniType.DAKKO);}
    private ItemStack oniArmor(Material material,String name){return registeredEquipment(oniArmorId(OniType.DAKKO,material.name().contains("HELMET")?"helmet":material.name().contains("CHESTPLATE")?"chestplate":material.name().contains("LEGGINGS")?"leggings":"boots"));}
    private int dakkoArmorModel(Material material){return switch(material){case DIAMOND_HELMET->3101;case DIAMOND_CHESTPLATE->3102;case DIAMOND_LEGGINGS->3103;case DIAMOND_BOOTS->3104;default->3101;};}
    private boolean isDakkoArmor(ItemStack i){if(i==null||!i.hasItemMeta())return false;String id=i.getItemMeta().getPersistentDataContainer().get(equipmentIdKey,PersistentDataType.STRING);return id!=null&&id.startsWith("oni.dakko.");}
    ...
    private ItemStack oniArmorWithModel(Material material,String name,int customModelData){ItemStack i=new ItemStack(material);ItemMeta meta=i.getItemMeta();meta.setDisplayName(cc(name));meta.setUnbreakable(true);meta.setCustomModelData(customModelData);i.setItemMeta(meta);return i;}
    private void equipShikkiArmor(Player p){equipOniArmor(p,OniType.SHIKKI);}
    private void equipYuukiArmor(Player p){equipOniArmor(p,OniType.YUUKI);}
    private void equipShikkiArmor(LivingEntity e){equipOniArmor(e,OniType.SHIKKI);}
    private void equipYuukiArmor(LivingEntity e){equipOniArmor(e,OniType.YUUKI);}
    private void equipJakutsukiArmor(Player p){equipOniArmor(p,OniType.JAKUTSUKI);}
    private ItemStack jakutsukiArmor(Material material,String name){String slot=material.name().contains("HELMET")?"helmet":material.name().contains("CHESTPLATE")?"chestplate":material.name().contains("LEGGINGS")?"leggings":"boots";return registeredEquipment(oniArmorId(OniType.JAKUTSUKI,slot));}
    private int jakutsukiArmorModel(Material material){return switch(material){case DIAMOND_HELMET->3601;case DIAMOND_CHESTPLATE->3602;case DIAMOND_LEGGINGS->3603;case DIAMOND_BOOTS->3604;default->3601;};}
    private boolean isJakutsukiArmor(ItemStack i){if(i==null||!i.hasItemMeta())return false;String id=i.getItemMeta().getPersistentDataContainer().get(equipmentIdKey,PersistentDataType.STRING);return id!=null&&id.startsWith("oni.jakutsuki.");}
